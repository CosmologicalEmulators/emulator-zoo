...
@everywhere function compute_class_observables(parameters)
    backend = ClassMnuW0WaGeneration.initialize_backend()
    return ClassMnuW0WaGeneration.compute_observables(parameters, backend)
end

# The callback is defined with @everywhere above, so every distributed worker
# has both the CLASS module and the callback before computation starts.
design = ClassMnuW0WaGeneration.create_design(n; seed=a["seed"])
start = time()
dataset_file = EmulatorsTrainer.compute_dataset_hdf5(
    design,
    ClassMnuW0WaGeneration.PARAMETER_NAMES,
    output,
