...
function main()
    ...
    length(prediction) == 9998 || error("Unexpected TT prediction length: $(length(prediction))")
    all(isfinite, prediction) || error("TT prediction contains NaN or Inf")
    println("CLASS Mnu-w0-wa smoke test passed: 50 samples, finite TT prediction length 9998")
end
