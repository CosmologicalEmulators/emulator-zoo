...
function load_camb_training_frame(path, spectrum, nodes, log_target)
    ...
    h5open(path, "r") do file
        ...
        result = DataFrame(
            ...
            omega_b=Float64[], omega_c=Float64[], Mnu=Float64[], OmegaK=Float64[],
        ...
        for sample_index in axes(parameters_array, 1)
            ...
            push!(result, (
                ...
                Mnu=Float64(parameters["Mnu"]), OmegaK=Float64(parameters["OmegaK"]),
                observable=target,
            ))
        end
        return result
...
function main()
    ...
    data_directory = length(ARGS) >= 2 ? abspath(ARGS[2]) :
        joinpath(@__DIR__, "data", "camb_mnuoklcdm_1000", "dataset.h5")
    output_root = length(ARGS) >= 3 ? abspath(ARGS[3]) :
        joinpath(@__DIR__, "artifacts", "camb_mnuoklcdm_1000")
