...
using .VelocileptorsLPTMnuOmegaKGeneration
@everywhere using EmulatorsTrainer, PyCall, Random
@everywhere include($GENERATION_FILE)
@everywhere using .VelocileptorsLPTMnuOmegaKGeneration
@everywhere function compute_velocileptors_observables(parameters)
    backend = VelocileptorsLPTMnuOmegaKGeneration.initialize_backend()
    return VelocileptorsLPTMnuOmegaKGeneration.compute_observables(parameters, backend)
end

design = VelocileptorsLPTMnuOmegaKGeneration.create_design(n_samples; seed=arguments["seed"])
start = time()
dataset_file = EmulatorsTrainer.compute_dataset_hdf5(
    design,
    VelocileptorsLPTMnuOmegaKGeneration.PARAMETER_NAMES,
    output_directory,
...
metadata = Dict(
    ...
    "parameter_names" => VelocileptorsLPTMnuOmegaKGeneration.PARAMETER_NAMES,
    "lower_bounds" => VelocileptorsLPTMnuOmegaKGeneration.LOWER_BOUNDS,
    "upper_bounds" => VelocileptorsLPTMnuOmegaKGeneration.UPPER_BOUNDS,
    "seed" => arguments["seed"],
