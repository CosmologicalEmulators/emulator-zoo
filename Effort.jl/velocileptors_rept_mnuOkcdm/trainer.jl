...
NN_dict = Dict{String,Any}("n_input_features"=>n_input_features,"n_output_features"=>n_output_features,"n_hidden_layers"=>5,"emulator_description"=>Dict("source"=>"CLASS + Velocileptors REPT","cosmology"=>"Mnu-OmegaK-CDM","component"=>Componentkind,"multipole"=>ℓ),"layers"=>Dict("layer_$i"=>Dict("n_neurons"=>64,"activation_function"=>"tanh") for i in 1:5))
mlpd = AbstractCosmologicalEmulators._get_nn_simplechains(NN_dict);

X, Y, Xtest, Ytest = EmulatorsTrainer.getdata(df; input_columns=Symbol.(array_pars_in), seed=20260747);
