...
using .VelocileptorsREPTMnuOmegaKGeneration
@everywhere using EmulatorsTrainer, PyCall, Random
@everywhere include($GENERATION_FILE)
@everywhere using .VelocileptorsREPTMnuOmegaKGeneration
@everywhere function compute_velocileptors_observables(parameters)
    backend = VelocileptorsREPTMnuOmegaKGeneration.initialize_backend()
    return VelocileptorsREPTMnuOmegaKGeneration.compute_observables(parameters, backend)
end

design = VelocileptorsREPTMnuOmegaKGeneration.create_design(n_samples; seed=arguments["seed"])
start = time()
dataset_file = EmulatorsTrainer.compute_dataset_hdf5(
    design,
    VelocileptorsREPTMnuOmegaKGeneration.PARAMETER_NAMES,
    output_directory,
...
metadata = Dict(
    ...
    "parameter_names" => VelocileptorsREPTMnuOmegaKGeneration.PARAMETER_NAMES,
    "lower_bounds" => VelocileptorsREPTMnuOmegaKGeneration.LOWER_BOUNDS,
    "upper_bounds" => VelocileptorsREPTMnuOmegaKGeneration.UPPER_BOUNDS,
    "seed" => arguments["seed"],
