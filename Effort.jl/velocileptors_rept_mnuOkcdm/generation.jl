module VelocileptorsREPTMnuOmegaKGeneration
...
function create_design(n;seed=20260746)
    Random.seed!(seed); create_training_dataset(n,LOWER_BOUNDS,UPPER_BOUNDS)
end
...
function compute_observables(p,b::Backend)
    ...
    try
        c.set(Dict("output"=>"mPk","P_k_max_h/Mpc"=>20.0,"z_pk"=>"0.0,3.0","h"=>h,"omega_b"=>p["ombh2"],"omega_cdm"=>p["omch2"],"ln10^{10}A_s"=>p["ln10As"],"n_s"=>p["ns"],"tau_reio"=>.0568,"N_ur"=>2.033,"N_ncdm"=>1,"m_ncdm"=>p["Mν"],"use_ppf"=>"yes","w0_fld"=>-1.0,"wa_fld"=>0.0,"fluid_equation_of_state"=>"CLP","cs2_fld"=>1.0,"Omega_k"=>p["Omega_k"],"Omega_Lambda"=>0.0)); c.compute()
