...
function get_observable_tuple(cosmo_pars, Pk)
    ...
    return (cosmo_pars["z"], cosmo_pars["ln10As"], cosmo_pars["ns"], cosmo_pars["H0"],
        cosmo_pars["ombh2"], cosmo_pars["omch2"], cosmo_pars["Mν"], reshape_Pk(Pk, factor))
end
...
df = DataFrame(z=Float64[], ln10A_s=Float64[], ns=Float64[], H0=Float64[], omega_b=Float64[], omega_cdm=Float64[], Mν=Float64[], observable=Array[])
@time EmulatorsTrainer.load_df_directory!(df, PℓDirectory, add_observable!)

array_pars_in = ["z", "ln10A_s", "ns", "H0", "omega_b", "omega_cdm", "Mν"]
